...
function ProductCard({ product, isSearchFromHome }: IProductProps) {
  ...
  const handleAddToCart = () => {
    let selectedProduct = cartStore.find(product => product.id === productCart.id);
    ...
    let updatedCart = cartStore.filter(product => product.id !== selectedProduct?.id);
  ...
  <Card key={product.id} className={ location.pathname === '/catalogue'  ? "col-lg-3 col-md-5 col-sm-10 col-11 m-4 p-4"  : "col-12 col-md-2 card_product"} >
    ...
    {
     (isSearchFromHome || productsByDateStore.length ) ? (<Button className="fs-2 p-3 col-7 m-auto" onClick={handleAddToCart}>Ajouter au panier</Button>) : ""
    }
  </Card>
  )
}
